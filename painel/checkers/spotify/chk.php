...
if (isset($_GET['email']) && isset($_GET['senha'])) {
    $c = new spotify();
    $json = json_decode($c->check($_GET['email'], $_GET['senha']));

    if ($json->status == "success") {
        if ($json->subscription == "PREMIUM") {
            echo "1LOGADO|" . $_GET['email'] . "|" . $_GET['senha'] . "|" . $json->subscription;
        } else {
            echo "0LOGADO|" . $_GET['email'] . "|" . $_GET['senha'];
        }
    } else {
        echo "0LOGADO|" . $_GET['email'] . "|" . $_GET['senha'];
    }
